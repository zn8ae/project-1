...
class grader(object):
    ...
    def reportScores(self):
        for key, val in self.scores.iteritems():
            print 'Test: {0}. Result {1}'.format(key, val)
        print 'Sum result: {0} out of {1}'.format(sum([x for x in self.scores.itervalues()]), len(self.scores))
